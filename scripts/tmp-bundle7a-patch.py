...
# Shared local-market contract -> v2 with crossVenue.
...
# Local intelligence service composes Coinbase observations.
p = Path('src/main/market/intelligence/local-market.ts')
text = p.read_text(encoding='utf-8')
if "coinbase-spot-service" not in text:
    text = text.replace(
...
text = text.replace(
    """  constructor(
    private readonly leadService: LeadCoreMarketService,
    private readonly altService: AltMarketService,
  ) {}""",
    """  constructor(
    private readonly leadService: LeadCoreMarketService,
    private readonly altService: AltMarketService,
    private readonly coinbaseService: CoinbaseSpotMarketService,
  ) {}""",
    1,
)
text = text.replace(
    """    const lead = this.leadService.getObservations(generatedAt);
    const altMarket = this.altService.buildIntelligence(generatedAt);
    const evidenceHealth = [
      ...this.leadService.getEvidenceHealth(generatedAt),
      ...this.altService.getEvidenceHealth(generatedAt),
    ];
    return buildLocalDecisionMarketIntelligence({
      generatedAt,
      leadCore: lead,
      altMarket,
      evidenceHealth,
    ...
    """    const lead = this.leadService.getObservations(generatedAt);
    const altMarket = this.altService.buildIntelligence(generatedAt);
    const coinbase = this.coinbaseService.getObservations(generatedAt);
    ...
    const evidenceHealth = [
      ...this.leadService.getEvidenceHealth(generatedAt),
      ...this.altService.getEvidenceHealth(generatedAt),
      ...this.coinbaseService.getEvidenceHealth(generatedAt),
    ];
    return buildLocalDecisionMarketIntelligence({
      generatedAt,
      leadCore: lead,
...
# Electron lifecycle starts/stops Coinbase public sensors independently.
p = Path('src/main/index.ts')
text = p.read_text(encoding='utf-8')
if "coinbase-spot-service" not in text:
    text = text.replace(
...
text = text.replace(
    """const altMarketService = new AltMarketService();
const localMarketIntelligence = new LocalMarketIntelligenceService(
  leadCoreMarketService,
  altMarketService,
);""",
    """const altMarketService = new AltMarketService();
const coinbaseSpotMarketService = new CoinbaseSpotMarketService();
const localMarketIntelligence = new LocalMarketIntelligenceService(
  leadCoreMarketService,
  altMarketService,
  coinbaseSpotMarketService,
);""",
    1,
)
text = text.replace(
    """  leadCoreMarketService.start();
  altMarketService.start();""",
    """  leadCoreMarketService.start();
  altMarketService.start();
  coinbaseSpotMarketService.start();""",
    1,
)
text = text.replace(
    """  marketData.stop();
  leadCoreMarketService.stop();
  altMarketService.stop();""",
    """  marketData.stop();
  leadCoreMarketService.stop();
  altMarketService.stop();
  coinbaseSpotMarketService.stop();""",
    1,
)
p.write_text(text, encoding='utf-8')

# OpenAPI local-market v2. Keep detailed crossVenue schema strict at the structural layer.
...
compact_returns = {
    ...
    'required': ['1m', '3m', '5m'],
    'properties': {'1m': nullable_number, '3m': nullable_number, '5m': nullable_number},
}
asset_schema = {
    'type': 'object',
    'additionalProperties': False,
    ...
    'properties': {
        ...
        'coinbaseSpot': {
            ...
            'properties': {
                'lastPrice': nullable_number, 'bidPrice': nullable_number, 'askPrice': nullable_number,
                'spreadBps': nullable_number, 'returnsBps': compact_returns,
                'normalizedTakerDelta1m': nullable_number, 'normalizedTakerDelta5m': nullable_number,
                'depthImbalance20': nullable_number, 'microPrice': nullable_number,
            },
        },
        'binancePerp': {
            'type': 'object', 'additionalProperties': False,
            ...
            'properties': {
                'markPrice': nullable_number, 'returnsBps': compact_returns,
                'normalizedTakerDelta1m': nullable_number, 'normalizedTakerDelta5m': nullable_number,
            },
        },
        'derived': {
...
local['required'] = [
    'version','generatedAt','objectiveOnly','leadCore','altMarket','crossVenue','evidenceHealth','provenance'
]
...
# GPT instructions: distinguish fast local crossVenue from coarse Worker crossMarket.
p = Path('worker/openapi/GPT_INSTRUCTIONS.md')
text = p.read_text(encoding='utf-8')
old = "- `cryptoMarket`: 로컬 ETH/SOL lead-core + 고정/Dynamic alt market의 객관 관측·파생통계. 방향 신호가 아니다."
new = "- `cryptoMarket`: 로컬 ETH/SOL·alt + `crossVenue`(Coinbase BTC/ETH/SOL spot vs Binance perp)의 객관 관측. `perpSpotReferenceSpreadBps`는 USD/USDT quote 차이를 포함한 참고값이며 arbitrage/방향 신호가 아니다."
if old not in text:
    raise SystemExit('GPT cryptoMarket anchor missing')
text = text.replace(old, new, 1)
old2 = "- `crossMarket`: Binance/Coinbase BTC/ETH/SOL 등 기존 cross-market corroboration. 상대강도/spread는 자동 방향 신호가 아니다."
new2 = "- `crossMarket`: Worker의 저빈도/광역 corroboration. 로컬 `cryptoMarket.crossVenue`와 중복되면 더 신선한 provenance/age를 우선하되 둘을 독립 신호처럼 이중계산하지 않는다."
if old2 not in text:
    raise SystemExit('GPT crossMarket anchor missing')
...
addition = anchor + """
    expect(json.components.schemas.LocalMarketIntelligence.properties.version.enum).toEqual([
      'local-market-v2',
    ]);
    expect(json.components.schemas.LocalMarketIntelligence.required).toContain(
      'crossVenue',
    );
    expect(json.components.schemas.CrossVenueIntelligence.additionalProperties).toBe(
      false,
    );
