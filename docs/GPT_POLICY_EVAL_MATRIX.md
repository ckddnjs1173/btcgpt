# GPT Policy v3 Evaluation Matrix

## Purpose

This matrix defines behavioral contracts for evaluating `gpt-policy-v3` on frozen `decision-context-v1` cases. It is not a trading strategy, does not define numeric market thresholds, and must not be converted into a local LONG/SHORT scoring engine.

Use it to review whether a GPT policy change altered the intended decision semantics before interpreting outcome metrics. The frozen `gpt-policy-v2` baseline is preserved at `research/gpt-policies/gpt-policy-v2.md` for matched replay comparison.
...
| G03 | Clear one-sided BTC structure, at least one independent current confirmation, definable invalidation, no material conflict, no chase | `ENTER_NOW` may be considered provisionally; same-snapshot `validateTradePlan` must succeed before final ENTER output                               | ENTER from auxiliary evidence alone or before validation                                          |
...
| G05 | One-sided BTC thesis exists, but one material confirmation is pending and a precise trigger/invalidation can be stated               | `WAIT_TRIGGER` with exactly one GPT-authored trigger contract and LONG/SHORT side matching that thesis                                              | Vague WAIT, NEUTRAL WAIT, or two-sided trigger fishing                                            |
...
| G09 | WAIT trigger later reports `TRIGGERED`                                                                                               | Call a fresh `getDecisionSnapshot` and re-analyze from scratch; only a new validated decision can become `ENTER_NOW`                                | Treating `TRIGGERED` as entry authorization                                                       |
...
| G15 | GPT forms an ENTER candidate, calls `validateTradePlan`, and deterministic validation blocks it                                      | Do not output an ENTER plan. Reclassify the final action from current evidence and record `planValidation=BLOCKED`                                  | Erasing the validation attempt as `NOT_APPLICABLE`, or leaving final decision as executable ENTER |

## Review dimensions

For each policy candidate, review these dimensions separately rather than producing one winner score:

1. **Decision semantics:** Are G01-G15 respected?
...
6. **Validation-block behavior:** count provisional ENTER candidates that were blocked separately from decisions where validation was never attempted.
7. **Management path quality:** protection coverage, subsequent MFE/MAE, exit/adjustment path behavior.
8. **Cohorts:** completeness and descriptive regime cohorts from the frozen input.
9. **Operational cost:** latency and reported API cost.
...
- identify which G01-G15 scenarios it is intended to affect;
- preserve the previous policy text for matched replay comparison;
