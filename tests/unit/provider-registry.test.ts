...
describe('ExternalProviderRegistry', () => {
  it('isolates optional provider failures and preserves successful evidence', async () => {
    const registry = new ExternalProviderRegistry([
      provider('GOOD', 'ETF', async (now = 1_000) => ({
        providerId: 'GOOD',
        kind: 'ETF',
        generatedAt: now,
      ...
      provider('BAD', 'ESTIMATED_LIQUIDATION', async () => {
  ...
  it('marks later failures degraded after a successful observation', async () => {
    ...
    const registry = new ExternalProviderRegistry([
      provider('OPTIONAL', 'ONCHAIN', async (now = 1_000) => {
        if (shouldFail) throw new Error('TEMPORARY_FAILURE');
        return {
          providerId: 'OPTIONAL',
          kind: 'ONCHAIN',
          generatedAt: now,
  ...
  it('rejects duplicate IDs and provider identity mismatches', async () => {
    const registry = new ExternalProviderRegistry();
    registry.register(
      provider('ONE', 'DERIVATIVES_AGGREGATE', async (now = 1_000) => ({
        providerId: 'WRONG',
